 ...
 * can only edit files declared in the active task, and is checked for
 * deviations at the end of each turn.
 *
 * Rules:
 *   Rule 1: Must define tasks via tilldone({ action: "define" }) before
 *           any write/edit/bash write operations.
 *   Rule 2: After starting a task, only files declared for that task
 *           may be written to or edited.
 *   Rule 3: On turn_end, if an active task is running and no declared
 *           files were touched, a reminder is shown.
 *
 * Configuration:
...
interface TilldoneState {
  /** Whether tasks have been defined for this session */
  defined: boolean;
  /** All tasks */
  tasks: TilldoneTask[];
  /** Currently active task index (0-based), -1 if none */
  activeIdx: number;
...
/** Bash write operations to check */
...
function isDeclaredFile(filePath: string, declaredFiles: string[]): boolean {
  if (declaredFiles.length === 0) return true; // no files declared = allow any
...
export default function (pi: ExtensionAPI) {
  ...
  // Tilldone is default-OFF — it's strict and requires explicit opt-in
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (!isWrite && !isBashWriteOp) return; // not a write operation

    // Rule 1: Must define tasks first
    if (!state.defined) {
      return {
        block: true,
    ...
    // Rule 2: Check file scope only for write/edit
    if (isWrite) {
      ...
      if (declaredFiles.length === 0) return; // no files declared = allow
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      switch (action) {
        ...
        case "start": {
          if (!state.defined) {
            return { content: [{ type: "text", text: "Call define first: tilldone({ action: \"define\", tasks: [...] })" }], details: {} };
          }
          if (!id || id < 1 || id > state.tasks.length) {
            return {
              content: [{ type: "text", text: `Invalid task ID: ${id}. Valid range: 1-${state.tasks.length}` }],
              details: {},
            };
          }
        ...
        case "complete": {
          ...
          if (targetIdx < 0 || targetIdx >= state.tasks.length) {
            return {
              content: [{ type: "text", text: `No active task. Use tilldone({ action: "start", id: N }) first.` }],
              details: {},
            };
          }
          ...
          return {
            content: [{
              type: "text",
              text: `✅ Task ${targetIdx + 1} completed: ${t.title} (${doneCount}/${state.tasks.length} done)${nextMsg}`,
            }],
            details: {},
          };
        }
        ...
        case "status": {
          if (!state.defined) {
            return { content: [{ type: "text", text: "No tasks defined. Use tilldone({ action: \"define\", tasks: [...] })." }], details: {} };
          }
          ...
          return {
            content: [{
              type: "text",
              text: `Tilldone Status (${state.statuses.filter(s => s === "done").length}/${state.tasks.length} done):\n${lines.join("\n")}`,
            }],
            details: {},
          };
        }
        ...
        default:
          return {
            content: [{ type: "text", text: `Unknown action: ${action}. Use define, start, complete, or status.` }],
            details: {},
          };
      }
    },
